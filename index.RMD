...
```{r total_kids_spending, echo=FALSE}
kable(total_kids_spending)
```

Calculates the state with the highest minimum wage from every year in the dataset 

```{r highest_each_state, echo=FALSE}
kable(highest_each_state)
```

Calculates the state with the lowest minimum wage

```{r lowest_each_state, echo=FALSE}
kable(lowest_each_state)
```
